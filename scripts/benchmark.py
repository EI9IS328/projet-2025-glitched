...
TIME_REGEX_PART = r"(\d+\.\d+) seconds."
BYTES_REGEX_PART = r"(\d+) Bytes."
...
def benchmark(
    ...
    output_file.write(
        "ex|ey|ez|order|format|receivers|global|kernel|make_snapshots|make_sismos|output_sismos|total_snap_bytes|total_sismos_bytes\n"
    )
    ...
    for comb in combs_product:
        ...
        # TODO: extract measures
        with open(measures, "r") as measures_file:
            print(measures_file.read())
        # TODO: put in CSV
        os.remove(measures)
        done += 1
